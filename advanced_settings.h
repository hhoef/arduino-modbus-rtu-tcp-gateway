...
/****** IP Settings ******/
const bool DEFAULT_AUTO_IP = true;  // Default Auto IP setting (only used if ENABLE_DHCP)
...
#ifdef ENABLE_WIFI
#define mySerial Serial1  // ESP32-C3 uses Serial1 for RS485, Serial0 is for USB/debug
const uint16_t WIFI_CONNECT_TIMEOUT = 20000;  // WiFi connection timeout in milliseconds
#else
...
#ifdef ENABLE_WIFI
#define RS485_CONTROL_PIN 2              // ESP32-C3 Pin for RS485 Direction control
#else
...
const uint16_t CHECK_ETH_INTERVAL = 2000;        // Interval (ms) to check SPI connection with ethernet shield (or WiFi connection)
...
const uint16_t TCP_DISCON_TIMEOUT = 500;         // Timeout (ms) for client DISCON socket command, non-blocking alternative to https://www.arduino.cc/reference/en/libraries/ethernet/client.setconn[...]  

#ifndef ENABLE_WIFI
const uint16_t TCP_RETRANSMISSION_TIMEOUT = 50;  // Ethernet controller's timeout (ms), blocking
const byte TCP_RETRANSMISSION_COUNT = 3;         // Number of transmission attempts the Ethernet controller will make before giving up
#endif
