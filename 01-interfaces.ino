...
/**************************************************************************/
/*!  
  @brief Initiates WiFi interface (for ESP32-C3) or Ethernet interface.
  If DHCP enabled, gets IP from DHCP, starts all servers (UDP, web server).
*/
/**************************************************************************/
void startEthernet() {
#ifdef ENABLE_WIFI
...
/**************************************************************************/
/*!  
  @brief Initiates WiFi interface for ESP32-C3
*/
/**************************************************************************/
#include <WiFi.h>
void startWiFi() {
  ...
  if (data.config.enableDhcp) {
    WiFi.begin(data.config.ssid, data.config.password);
  ...
  // If DHCP failed, use static IP
  if (!dhcpSuccess) {
    IPAddress staticIP(data.config.ip[0], data.config.ip[1], data.config.ip[2], data.config.ip[3]);
    IPAddress gateway(data.config.gateway[0], data.config.gateway[1], data.config.gateway[2], data.config.gateway[3]);
    IPAddress subnet(data.config.subnet[0], data.config.subnet[1], data.config.subnet[2], data.config.subnet[3]);
...
/**************************************************************************/
/*!  
  @brief Initiates Ethernet shield interface
*/
/**************************************************************************/
...
/**************************************************************************/
/*!  
  @brief Resets Arduino (works only on AVR chips and ESP32).
*/
/**************************************************************************/
...
/**************************************************************************/
/*!  
  @brief Checks SPI connection to the W5X00 chip or WiFi connection.
*/
/**************************************************************************/
void checkEthernet() {
#ifdef ENABLE_WIFI
...
void manageSockets() {
#ifdef ENABLE_WIFI
  // WiFi on ESP32 handles sockets differently, simpler management needed
  uint32_t maxAge = 0;
  byte oldest = MAX_SOCK_NUM;
  byte dataAvailable = MAX_SOCK_NUM;
  
  // Basic socket management for WiFi - much simpler than Ethernet shield
  // The underlying WiFi stack handles most of this automatically
...
#else
// For ESP32, use built-in random
void CreateTrulyRandomSeed() {
  seed1 = random(0xFFFFFFFF);
  seed2 = random(0xFFFFFFFF);
}
#endif
...
#define BOARD F("ESP32-C3 DevKit")
